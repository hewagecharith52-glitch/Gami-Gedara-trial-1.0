...
export default function OrderTracking({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-20 selection:bg-orange-500/30">
      ...
      <header className="fixed top-0 w-full z-50 bg-white/95 backdrop-blur-md border-b border-slate-200 px-4 sm:px-6 h-[72px] flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            <h1 className="text-slate-900 font-bold text-sm sm:text-base leading-tight truncate">{settings?.name || restaurantConfig.name}</h1>
            <p className="text-[8px] sm:text-[10px] text-orange-500 font-bold uppercase tracking-wider">Smart QR Menu</p>
          </div>
        </div>
        <div className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 bg-slate-50 rounded-xl border border-slate-200 shrink-0">
          {isConnected ? (
            <><Wifi className="w-3 h-3 text-emerald-500" /><span className="text-[9px] sm:text-[10px] font-bold text-slate-600 uppercase tracking-widest">Live</span></>
          ) : (
            <><WifiOff className="w-3 h-3 text-slate-400" /><span className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest animate-pulse">Syncing</span></>
          )}
        </div>
      </header>

      <main className="max-w-md mx-auto px-3 sm:px-5 pt-24">
        {/* Header Section */}
        <div className="text-center mb-6 sm:mb-8">
          <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px] sm:text-xs mb-1.5">Live Tracking</p>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">Order #{order.id.slice(0, 5).toUpperCase()}</h1>
        </div>

        {/* Status Card - Responsive padding & Timeline circle spacing */}
        <div className="bg-white border border-slate-100 rounded-[2rem] p-4 sm:p-6 shadow-md shadow-slate-200/50 mb-6 sm:mb-8 relative overflow-hidden">
          {/* Progress Timeline */}
          <div className="relative mb-6 sm:mb-8 mt-2 px-2">
            <div className={`absolute top-1/2 left-5 right-5 h-1 -translate-y-1/2 rounded-full z-0 ${normalizedStatus === "completed" ? "bg-emerald-500" : "bg-slate-100"}`}></div>
            <div
              className={`absolute top-1/2 left-5 h-1 -translate-y-1/2 rounded-full z-0 transition-all duration-700 ease-out ${normalizedStatus === "completed" ? "bg-emerald-500" : "bg-orange-500"}`}
              style={{ width: `calc(${currentStepIdx < 0 ? 0 : (currentStepIdx / (steps.length - 1)) * 100}% - 10px)` }}
            ></div>
            ...
            <div className="relative z-10 flex justify-between">
              {steps.map((step, idx) => {
                ...
                let iconWrapperClass = `w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center transition-all duration-500 border-2 border-white relative `;
                ...
                if (normalizedStatus === "completed") {
                  ...
                  iconWrapperClass += `bg-gradient-to-br from-amber-500 to-orange-500 text-white ring-2 sm:ring-4 ring-orange-400/40 shadow-lg shadow-orange-500/30 animate-pulse `;
                } else if (isActive) {
                  if (isCurrent) {
                    if (step === "ready") {
                      iconWrapperClass += `bg-emerald-500 text-white ring-2 sm:ring-4 ring-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.5)] `;
                    } else if (step === "pending") {
                      iconWrapperClass += `bg-amber-500 text-white animate-pulse ring-2 sm:ring-4 ring-amber-500/30 shadow-[0_0_15px_rgba(245,158,11,0.3)] `;
                    }
                  } else {
                ...
                return (
                  <div key={step} className="flex flex-col items-center gap-1.5 sm:gap-2 relative">
                    <div className={iconWrapperClass}>
                      <Icon className={`w-4 h-4 sm:w-5 sm:h-5 relative z-10 ${normalizedStatus === "completed" ? "animate-pop" :
                      ...
                      {step === "preparing" && isPendingOrPreparing && (
                        <div className="absolute -top-3.5 sm:-top-4 flex items-end justify-center gap-0.5 sm:gap-1 w-full pointer-events-none">
                          <span className="w-1 h-2.5 sm:h-3 bg-orange-400 rounded-full animate-steam opacity-80"></span>
                          <span className="w-1.5 h-3.5 sm:h-4 bg-amber-400 rounded-full animate-steam opacity-90 [animation-delay:100ms]"></span>
                          <span className="w-1 h-2 sm:h-2.5 bg-orange-300 rounded-full animate-steam opacity-70 [animation-delay:200ms]"></span>
                        </div>
                      )}
          ...
          <div className="text-center">
            <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-1">{normalizedStatus === "pending" ? "Order Received" : (normalizedStatus === "preparing" ? "Preparing in Kitchen 🍳" : order.status)}</h2>
          ...
          {(normalizedStatus === "pending" || normalizedStatus === "preparing") && (
            <div className="mt-6 sm:mt-8 bg-slate-50 rounded-2xl p-4 sm:p-5 border border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2.5 sm:gap-3">
                <div className="w-9 h-9 sm:w-10 sm:h-10 bg-white rounded-xl shadow-sm border border-slate-200 flex items-center justify-center shrink-0">
                  <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-orange-500" />
                </div>
                <div>
                  <p className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest">Est. Prep Time</p>
                  <p className="text-slate-900 font-bold text-sm sm:text-base">{prepTimeMinutes} mins</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mb-0.5">Time Left</p>
                <p className="text-lg sm:text-xl font-bold text-slate-900 tabular-nums">{timeLeftStr || "..."}</p>
              </div>
            </div>
          )}
        </div>
        ...
        {/* Order Details */}
        <div className="bg-white border border-slate-100 rounded-3xl p-4 sm:p-6 shadow-sm mb-6 sm:mb-8">
          ...
          <div className="space-y-3">
            {order.items.map((item, idx) => (
              <div key={idx} className="flex flex-col border-b border-slate-50 pb-3 last:border-0 last:pb-0">
                <div className="flex justify-between items-start text-xs sm:text-sm font-medium">
                  <div className="flex gap-2 sm:gap-3">
                    <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-md bg-slate-100 text-slate-600 flex items-center justify-center text-[11px] sm:text-xs font-bold shrink-0 mt-0.5">{item.quantity}x</span>
                    <div className="flex flex-col">
                      <span className="text-slate-700 font-bold">{item.name}</span>
                      {item.notes && <span className="text-[10px] sm:text-xs text-slate-400 italic mt-0.5">Note: {item.notes}</span>}
                    </div>
                  </div>
            ...
            {(() => {
              ...
              return (
                <div className="pt-3 border-t border-slate-100 mt-2 space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-slate-100 mt-2">
                    <span className="font-bold text-slate-500 uppercase tracking-widest text-[10px] sm:text-xs mt-1">Grand Total</span>
                    <span className="text-lg sm:text-xl font-bold text-slate-900">{currencySymbol} {(order.total_amount ?? (subtotal + serviceCharge + tax)).toLocaleString()}</span>
                  </div>
                </div>
              );
            })()}
        ...
        <div className="border border-indigo-200 bg-gradient-to-r from-indigo-50/70 via-purple-50/50 to-pink-50/70 shadow-md shadow-indigo-100 rounded-3xl p-1 mb-6 sm:mb-8 overflow-hidden touch-pan-y" style={{ touchAction: 'pan-y' }}>
          <button
            onClick={() => setShowGame(!showGame)}
            className="w-full flex items-center justify-between p-3 sm:p-4 hover:bg-white/50 transition-colors rounded-2xl"
          >
            <div className="flex items-center gap-2.5 sm:gap-3">
              <div className="w-9 h-9 sm:w-10 sm:h-10 bg-indigo-100 rounded-xl flex items-center justify-center text-indigo-600 shadow-inner shrink-0">
                <Gamepad2 className="w-4 h-4 sm:w-5 sm:h-5 animate-bounce" style={{ animationDuration: '1.5s' }} />
              </div>
              <div className="text-left min-w-0">
                <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                  <h3 className="font-bold text-slate-900 text-xs sm:text-base leading-tight">Play While You Wait</h3>
                  <span className="px-1.5 sm:px-2 py-0.5 text-[8px] sm:text-[9px] font-black uppercase tracking-wider bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-full animate-pulse shadow-sm shadow-indigo-500/30 whitespace-nowrap">🔥 Play & Chill</span>
                </div>
                <p className="text-[10px] sm:text-xs text-slate-500 font-medium mt-0.5 truncate">Tap to smash bricks & kill waiting time</p>
              </div>
            </div>
            <div className="flex items-center gap-1 sm:gap-2 shrink-0 pl-1.5 sm:pl-2">
              {!showGame && <span className="text-[9px] sm:text-[10px] font-bold text-indigo-600 uppercase tracking-widest bg-indigo-100 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg">Open</span>}
              {showGame ? <ChevronUp className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-400" /> : <ChevronDown className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-400" />}
            </div>
          </button>

          {showGame && (
            <div className="p-2 sm:p-4 pt-1 sm:pt-2 animate-in slide-in-from-top-2 duration-300">
              <BrickBreaker />
            </div>
          )}
        </div>
        ...
        {!isCompleted ? (
          <Link
            ...
            className="w-full flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white font-bold py-3.5 sm:py-4 rounded-2xl transition-all active:scale-95 shadow-lg shadow-slate-900/20 text-sm sm:text-base"
          >
            <UtensilsCrossed className="w-4 h-4 sm:w-5 sm:h-5" /> Order More Items
          </Link>
        ) : (
          <button
            onClick={() => setIsReviewModalOpen(true)}
            className="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-3.5 sm:py-4 rounded-2xl shadow-lg active:scale-95 flex items-center justify-center gap-2 transition-all text-sm sm:text-base cursor-pointer"
          >
            <Star className="w-4 h-4 sm:w-5 sm:h-5 fill-white" /> Leave a Review ⭐
          </button>
        )}
      </main>
      ...
      {showReadyModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-sm rounded-[2rem] p-6 sm:p-8 text-center shadow-2xl animate-in zoom-in-95 duration-500">
            <div className="w-20 h-20 sm:w-24 sm:h-24 bg-orange-500 rounded-full flex items-center justify-center mx-auto mb-5 sm:mb-6 shadow-lg shadow-orange-500/30 ring-8 ring-orange-50">
              <BellRing className="w-10 h-10 sm:w-12 sm:h-12 text-white animate-bounce" />
            </div>
            <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-2 sm:mb-3">Order Ready!</h2>
            ...
            <button
              ...
              className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3.5 sm:py-4 rounded-2xl transition-all active:scale-95 text-sm sm:text-base"
            >
              Got it, thanks!
            </button>
          </div>
