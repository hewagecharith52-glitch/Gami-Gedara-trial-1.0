...
import {
  ...
  ChevronLeft, ChevronRight, ArrowLeft, Tag, QrCode, Grid
} from "lucide-react";
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<"menu" | "tables" | "settings">("menu");
  ...
  // Tables Manager State (Direct Supabase restaurant_tables Sync)
  ...
  const fetchTables = useCallback(async () => {
    try {
      ...
      console.warn("Failed to fetch tables in Admin:", e);
    }
  }, []);

  const handleAddTable = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newTableNo.trim() || isAddingTable) return;

    const trimmedNo = newTableNo.trim();
    if (dbTables.some((t) => String(t.table_no) === trimmedNo)) {
      showToast(`Table ${trimmedNo} already exists.`, "error");
      return;
    }
    ...
    try {
      ...
      if (error) {
        ...
        showToast(`Table ${trimmedNo} added successfully!`);
        setNewTableNo("");
        setNewTableCapacity("4");
        fetchTables();
      }
    } catch (err: any) {
      showToast("Error adding table: " + err.message, "error");
    } finally {
      setIsAddingTable(false);
    }
  };

  const handleDeleteTable = async (tableObj: any) => {
    if (!confirm(`Are you sure you want to delete Table ${tableObj.table_no}?`)) return;
    ...
    try {
      ...
      if (error) {
        ...
        showToast(`Table ${tableObj.table_no} deleted.`);
        fetchTables();
      }
    } catch (err: any) {
  ...
  }, [isUnlocked, fetchMenu, fetchTables]);
      ...
      <main className="min-h-screen bg-slate-50 flex flex-col pt-[72px]">
        ...
        <div className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 flex flex-col gap-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex items-center gap-2 bg-white rounded-xl p-1 shadow-sm border border-slate-200 w-full sm:w-auto overflow-x-auto no-scrollbar">
              <button
                ...
                className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 rounded-lg text-xs sm:text-sm font-bold transition-all whitespace-nowrap cursor-pointer ${activeTab === "menu" ? 'bg-orange-50 text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-900'
                  }`}
              >
              ...
              <button
                ...
                className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 rounded-lg text-xs sm:text-sm font-bold transition-all whitespace-nowrap cursor-pointer ${activeTab === "tables" ? 'bg-orange-50 text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-900'
                  }`}
              >
                <Grid className="w-4 h-4" /> Tables
              </button>

              <button
                onClick={() => setActiveTab("settings")}
                className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 rounded-lg text-xs sm:text-sm font-bold transition-all whitespace-nowrap cursor-pointer ${activeTab === "settings" ? 'bg-orange-50 text-orange-600 shadow-sm' : 'text-slate-500 hover:text-slate-900'
                  }`}
              >
          ...
          <div className="flex-1 flex flex-col">
            ...
            {/* TAB 2: Tables Manager (Direct restaurant_tables DB Sync) */}
            {activeTab === "tables" && (
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 flex-1 flex flex-col max-w-4xl mx-auto w-full overflow-hidden">
                <div className="p-4 sm:p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div>
                    <h2 className="text-base sm:text-lg font-bold text-slate-800 flex items-center gap-2">
                      <Grid className="w-5 h-5 text-orange-500" /> Restaurant Tables Manager
                    </h2>
                    <p className="text-xs sm:text-sm text-slate-500 mt-1">
                      Add, remove, and manage physical dining tables. Automatically synchronizes with QR Menu & Cashier.
                    </p>
                  </div>
                  <span className="bg-orange-100 text-orange-700 px-3 py-1 rounded-xl text-xs font-black shadow-xs">
                    {dbTables.length} Active Tables
                  </span>
                </div>

                <div className="p-4 sm:p-6 space-y-6">
                  {/* Add New Table Form */}
                  <form onSubmit={handleAddTable} className="bg-slate-50 p-4 sm:p-5 rounded-2xl border border-slate-200 flex flex-col sm:flex-row items-end gap-3">
                    <div className="flex-1 w-full">
                      ...
                      <input
                        ...
                        placeholder="e.g. 13 or 14"
                        className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 font-bold text-xs sm:text-sm text-slate-900 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20"
                        required
                      />
                    </div>
                    ...
                    <button
                      type="submit"
                      disabled={isAddingTable || !newTableNo.trim()}
                      className="w-full sm:w-auto bg-orange-500 hover:bg-orange-600 text-white font-bold px-6 py-2.5 rounded-xl text-xs sm:text-sm shadow-md shadow-orange-500/20 transition-all flex items-center justify-center gap-1.5 active:scale-95 disabled:opacity-50 shrink-0 cursor-pointer"
                    >
                  ...
                  <div>
                    <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Existing Tables in Database</h3>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 max-h-[420px] overflow-y-auto no-scrollbar p-1">
                      {dbTables.map((table) => {
                        const paddedNo = String(table.table_no).padStart(2, "0");
                        return (
                          <div
                            key={table.id}
                            className="bg-white rounded-2xl border-2 border-slate-200/80 p-3.5 flex flex-col items-center justify-between text-center relative group shadow-2xs hover:border-orange-300 hover:shadow-sm transition-all"
                          >
                            <button
                              type="button"
                              onClick={() => handleDeleteTable(table)}
                              className="absolute top-2 right-2 p-1.5 text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors cursor-pointer"
                              title={`Delete Table ${table.table_no}`}
                            >
                              <Trash2 className="w-3.5 h-3.5" />
                            </button>

                            <span className="text-2xl mb-1 mt-1">🍽️</span>
                            <span className="font-extrabold text-base text-slate-900 leading-tight">
                              Table {paddedNo}
                            </span>
                            <span className="text-[10px] font-bold text-slate-500 mt-1 bg-slate-100 px-2 py-0.5 rounded-full border border-slate-200">
                      ...
                      {dbTables.length === 0 && (
                        <div className="col-span-full py-12 text-center text-slate-400 font-bold text-sm">
                          No tables configured in database. Use the form above to add Table 01.
                        </div>
                      )}
                    </div>
                  </div>
